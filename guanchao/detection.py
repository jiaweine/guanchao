...
from .domain import AccountSnapshot, DetectionResult, Evidence, FeatureVector
...
class MarketingDetector(DetectionBehaviorMixin):
    ...
    def analyze(self, account: AccountSnapshot, media_text: str = "") -> DetectionResult:
        ...
        stability, _ = self.stability_probe(account, media_text)
        disclosure_gap = max(0.0, 1.0 - features.disclosure_signal)
        conversion_pressure = max(
            features.call_to_action,
            features.contact_pressure,
            features.cross_post_pressure,
            features.profile_commerciality,
        )
        conjunction = marketing * disclosure_gap * conversion_pressure
        covert = conjunction ** (1.0 / 3.0) if conjunction > 0.0 else 0.0
        confidence = self._confidence(
            account,
            features,
            stability,
            marketing,
            semantic.grounded_fraction,
        )
    ...
    def score(self, features: FeatureVector) -> float:
        ...
        for key, weight in self.calibration.interactions.items():
            ...
            if not pair:
                continue
            first = max(0.0, float(getattr(features, pair[0], 0.0)))
            second = max(0.0, float(getattr(features, pair[1], 0.0)))
            linear += weight * first * second
        scaled = linear / max(1e-6, self.calibration.temperature)
        return 1.0 / (1.0 + math.exp(-max(-20.0, min(20.0, scaled))))

    def extract(
        self, account: AccountSnapshot, media_text: str = ""
    ) -> tuple[FeatureVector, list[Evidence], list[str]]:
        features, evidence, missing, _ = self._extract(
            account, media_text, use_semantic=True
        )
    ...
    def _extract(
        ...
        *,
        ...
        authentic_variation = _clamp(authentic * math.sqrt(max(0.0, lexical_variation)))
        ...
        identity_consistency = _clamp(
            ...
            + math.sqrt(max(0.0, media_commerciality * content_operation))
            * (1.0 - math.sqrt(max(0.0, profile_commerciality * content_operation)))
        )
        ...
        if not any(
            post.likes or post.comments or post.shares or post.views for post in posts
        ):
        ...
        if not evidence and texts:
            positive_keys = [
                key
                for key, weight in self.calibration.weights.items()
                if weight > 0 and hasattr(features, key)
            ]
            peak = max(
                (float(getattr(features, key, 0.0)) for key in positive_keys),
                default=0.0,
            )
            evidence.append(
                Evidence(
    ...
    def _fuse_semantic(
        self, features: FeatureVector, semantic: SemanticSignal
    ) -> FeatureVector:
    ...
    def stability_probe(
        ...
        if not posts:
            ...
            return result
        base_features, _, _, _ = self._extract(
            account, media_text, use_semantic=False
        )
        ...
        for index in range(min(len(posts), 12)):
            reduced = AccountSnapshot(
                ...
                posts=[post for row, post in enumerate(posts) if row != index],
            )
            features, _, _, _ = self._extract(
                reduced, media_text, use_semantic=False
            )
            scores.append(self.score(features))
        max_swing = max((abs(score - base) for score in scores), default=0.0)
        spread = statistics.pstdev(scores) if len(scores) > 1 else 0.0
        influence = math.hypot(max_swing, spread) * math.sqrt(len(scores))
        stability = _clamp(math.exp(-influence))
